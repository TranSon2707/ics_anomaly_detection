...
def tuning_hyperparameter_LSTM():

    tuning_results = ""

    param_grid = [
        {'num_layers': 2, 'units_per_layer': [64, 32]},
        {'num_layers': 3, 'units_per_layer': [64, 48, 32]},
        {'num_layers': 4, 'units_per_layer': [64, 48, 32, 16]},
    ]
    ...
    X_attack_scaled_lstm = X_attack_scaled.reshape((X_attack_scaled.shape[0], X_attack_scaled.shape[1], 1))

    for config in param_grid:
        print(f"[INFO] Training with: num_layers = {config['num_layers']}, units = {config['units_per_layer']}")

        model = DL_model.build_lstm_autoencoder(
        ...
        model.compile(optimizer='adam', loss='mse')
        model.fit(X_train_lstm, X_train_lstm, epochs=50, batch_size=64, verbose=0)

        # Dự đoán dữ liệu tấn công
        X_attack_hat = model.predict(X_attack_scaled_lstm)
        
        # Đưa về cùng (sample_size, 43)
        # X_attack_hat = np.squeeze(X_attack_hat, axis=1)
        if X_attack_hat.shape[1] == 1:
            X_attack_hat = np.squeeze(X_attack_hat, axis=1)
        else:
            X_attack_hat = np.mean(X_attack_hat, axis=1)    # If there are multiple timesteps, average them
        print(f"[INFO] X_attack_hat.shape = {X_attack_hat.shape}")
    ...
    try:
        joblib.dump({'threshold': best_threshold, 'model': best_model}, 'saved_model/lstm_autoencoder/lstm_ae_best.save')
    except Exception as e:
        print(f"[Warning] Unable to save model: {e}")

    with open('src/result/tuning/tuning_hyperparameter_lstm_ae.txt', 'w') as f:
        f.write(tuning_results)
...
# tuning_hyperparameter_LSTM()

def best_model_retrive_lstm_ae():

    filepath = 'saved_model/lstm_autoencoder/lstm_ae_best.save'
    if not os.path.exists(filepath):
        print("Model not found.")
        return None
    else:
