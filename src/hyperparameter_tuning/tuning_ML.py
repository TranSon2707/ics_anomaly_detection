...
def tuning_hyperparameter_XGBoost():
    # Define the grid of hyperparameters to search
    max_depths = [2, 3, 4, 5]
    learning_rates = [0.01, 0.05, 0.1, 0.2]
    n_estimators_list = [50, 100, 200]

    tuning_result_XGBoost = ''
    best_model = None
    best_f1 = -1
    best_threshold = None
    best_params = {}
    ...
    for max_depth in max_depths:
        for learning_rate in learning_rates:
            for n_estimators in n_estimators_list:
                print(f"[INFO] Trying max_depth={max_depth}, learning_rate={learning_rate}, n_estimators={n_estimators}")
                model = ML_model.train_XGBoost(max_depth, learning_rate, n_estimators)
                ...
                print(f"max_depth={max_depth}, learning_rate={learning_rate}, n_estimators={n_estimators} | F1={f1:.4f} | threshold={threshold:.6f}")
                tuning_result_XGBoost += f"max_depth={max_depth}, learning_rate={learning_rate}, n_estimators={n_estimators} | F1={f1:.4f} | threshold={threshold:.6f}\n"
    ...
    print(f"Best F1 = {best_f1:.4f} | threshold = {best_threshold:.6f} | params = {best_params}")
    tuning_result_XGBoost += f"Best F1 = {best_f1:.4f} | threshold = {best_threshold:.6f} | params = {best_params}\n"

    try:
        joblib.dump({'threshold': best_threshold, 'model': best_model, 'params': best_params}, 'saved_model/xgboost/xgboost_best.save')
    except Exception as e:
        print(f"[Warning] Unable to save model: {e}")

    with open('src/result/tuning/tuning_hyperparameter_XGBoost.txt', 'w') as f:
        f.write(tuning_result_XGBoost)
...
def best_model_retrive_XGB():
    filepath = 'saved_model/xgboost/xgboost_best.save'
    if not os.path.exists(filepath):
        print("Model not found.")
    else:
        saved_model = joblib.load(filepath)
...
def tuning_hyperparameter_random_forest_regression():
    ...
    print(f"F1={f1:.4f} | threshold={threshold:.6f}")
    tuning_result_rf += f"F1={f1:.4f} | threshold={threshold:.6f}\n"
    ...
    tuning_result_rf += f"Best F1 = {best_f1:.4f} | threshold = {best_threshold:.6f}\n"

    try:
        joblib.dump({'threshold': best_threshold, 'model': best_model}, 'saved_model/random_forest/random_forest_best.save')
    except Exception as e:
        print(f"[Warning] Unable to save model: {e}")

    with open('src/result/tuning/tuning_hyperparameter_random_forest.txt', 'w') as f:
        f.write(tuning_result_rf)
...
def best_model_retrive_RF():
    filepath = 'saved_model/random_forest/random_forest_best.save'
    if not os.path.exists(filepath):
        print("Model not found.")
    else:
        saved_model = joblib.load(filepath)
